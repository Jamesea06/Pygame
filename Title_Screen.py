...
while running:
    ...
    if game_state == "start_menu":
        ...
        if start_menu_events():
            game_state = "game"
            # Initialize game objects here if needed
            player = Player()  # Example: Initialize player object
            continue  # Restart the loop to skip the "game" state logic until next iteration
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_ESCAPE:
                    running = False
        # Example: Update and render game objects
        screen.fill((0, 0, 0))  # Example: Clear screen
        # Example: Render player object
